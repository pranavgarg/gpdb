...
class BackupTestCase(TINCTestCase):
    ...
    def validate_backup_files_on_mounted_backedup_dir(self, NFSBACKUPDIR):
        #validate that all the segment and master dump files are available on the shared path NFSBACKUPDIR
        data_sql = "select content, dbid from gp_segment_configuration where role='p' and preferred_role='p' and content <> -1;"
        (rc, data_out) = self.run_SQLCommand(data_sql, dbname='bkdb1', flags='-t')
        if rc != 0:
            raise Exception("sql command \"%s\" failed with return code %s " % (data_sql, rc) )
        results = [s.split('|') for s in data_out.splitlines() if len(s) > 0]
        contents = [r[0].strip() for r in results]
        db_ids = [r[1].strip() for r in results]
        datestamp = self.get_latest_timestamp()[0:8]
        backup_dir_path = os.path.join(NFSBACKUPDIR,'db_dumps',datestamp)
        backup_dir_files = os.listdir(backup_dir_path)
        regexes=[]
        for i in range(len(contents)):
            compiled = re.compile('.*_[0%s]_%s_.*\.gz' % (contents[i], db_ids[i]))
            succeded = False
            for file in backup_dir_files:
                if compiled.match(file):
                    ...
                    break
            if not succeded:
                raise Exception("cannot find expected pattern %s " % compiled.pattern)
