...
void test__formDDBoostPsqlCommandLine13_with_storage_unit(void **state)
{
	...
	bool postSchemaOnly = false;
	const char* change_schema_file = "/tmp/change_schema_file";
	const char* storage_unit_name = "foo";
	...
	char *e = "ddboostPg --readFile --from-file=ddb_filename.gz --ddboost_storage_unit_name=foo --dd_boost_buf_size=512MB | gzip -c | filter.py -t filter.conf -c /tmp/change_schema_file | psql";
	assert_string_equal(cmdLine, e);
	free(cmdLine);
}
