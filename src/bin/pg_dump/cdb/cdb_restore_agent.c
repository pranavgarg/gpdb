...
main(int argc, char **argv)
{
...
#ifdef USE_DDBOOST
			case 19:
				ddboost_storage_unit_name = strdup(optarg);
				break;
#endif
			default:
				fprintf(stderr, _("Try \"%s --help\" for more information.\n"), progname);
	...
	if(dd_boost_enabled)
		cleanupDDSystem();
	if (ddboost_storage_unit_name)
		free(ddboost_storage_unit_name);
#endif
...
usage(const char *progname)
{
	...
	printf(_("  --ddboost-storage-unit-name     Storage unit to use on the ddboost server\n"));
