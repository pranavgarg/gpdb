...
@then('store the oid of table "{table}" from schema "{schema}" and database "{dbname}"')
@when('store the oid of table "{table}" from schema "{schema}" and database "{dbname}"')
@given('store the oid of table "{table}" from schema "{schema}" and database "{dbname}"')
def impl(context, table, dbname):
    table_oid = get_table_oid(context, dbname, schema, table)
    context.saved_table_oid = table_oid


#@then('the user runs "{command}" on a random segment in database "{dbname}")
#@when('the user runs "{command}" on a random segment in database "{dbname}")
#@given('the user runs "{command}" on a random segment in database "{dbname}")
#def impl(context, dbname, command):
#    #select * from gp_segment_configuration where content <> -1 order by random() limit 1;
#    pass
