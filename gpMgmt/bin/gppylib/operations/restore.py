...
def _build_gpdbrestore_cmd_line(ts, table_file, backup_dir, redirected_restore_db, report_status_dir, dump_prefix, ddboost=False, netbackup_service_host=None,
                                netbackup_block_size=None, change_schema=None, schema_level_restore_file=None, storage_unit_name=None):
    ...
    if storage_unit_name:
        cmd += " --ddboost_storage_unit_name=%s" % storage_unit_name
    if netbackup_service_host:
...
class RestoreDatabase(Operation):
    ...
    def _build_restore_line(self, restore_timestamp, restore_db, compress, master_port, no_plan,
        ...
        if self.storage_unit_name:
            restore_line += " --ddboost_storage_unit_name=%s" % self.storage_unit_name
...
class GetDDboostDumpTablesOperation(GetDumpTablesOperation):
    def __init__(self, restore_timestamp, master_datadir, backup_dir, dump_dir, dump_prefix, compress, dump_file, storage_unit_name=None):
    ...
    def execute(self):
        ...
        if self.storage_unit_name:
            ddboost_cmdStr += ' --storage_unit_name=%s' % self.storage_unit_name
...
class GetDumpTables():
    def __init__(self, restore_timestamp, master_datadir, backup_dir,
                        ...
                        dump_file=None, storage_unit_name=None):
        """
    ...
    def get_dump_tables(self):
        if self.ddboost:
            get_dump_table_cmd = GetDDboostDumpTablesOperation(self.restore_timestamp, self.master_datadir, self.backup_dir,
                                                                self.dump_dir, self.dump_prefix, self.compress, self.dump_file, self.storage_unit_name)
