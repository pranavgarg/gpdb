...
def backup_schema_file_with_ddboost(master_datadir, backup_dir, dump_dir, dump_prefix, timestamp_key=None, storage_unit_name=None):
    if timestamp_key is None:
        timestamp_key = TIMESTAMP_KEY

    filename = generate_schema_filename(master_datadir, backup_dir, dump_dir, dump_prefix, timestamp_key, True)
    copy_file_to_dd(filename, dump_dir, timestamp_key, storage_unit_name=storage_unit_name)

def backup_report_file_with_ddboost(master_datadir, backup_dir, dump_dir, dump_prefix, timestamp_key=None, storage_unit_name=None):
    if timestamp_key is None:
        timestamp_key = TIMESTAMP_KEY
...
def backup_increments_file_with_ddboost(master_datadir, backup_dir, dump_dir, dump_prefix, full_timestamp, storage_unit_name=None):
    filename = generate_increments_filename(master_datadir, backup_dir, dump_dir, dump_prefix, full_timestamp, True)
    copy_file_to_dd(filename, dump_dir, full_timestamp, storage_unit_name=storage_unit_name)

def copy_file_to_dd(filename, dump_dir, timestamp_key=None, storage_unit_name=None):
    if timestamp_key is None:
        timestamp_key = TIMESTAMP_KEY
    ...
    if storage_unit_name:
        cmdStr += " --storage_unit_name=%s" % storage_unit_name
...
def write_state_file(table_type, master_datadir, backup_dir, dump_dir, dump_prefix, partition_list, ddboost=False, ddboost_storage_unit=None):
    ...
    if ddboost:
        copy_file_to_dd(filename, dump_dir, storage_unit_name=ddboost_storage_unit)
...
def write_dirty_file(mdd, dirty_tables, backup_dir, dump_dir, dump_prefix, timestamp_key=None, ddboost=False, ddboost_storage_unit=None):
    ...
    if ddboost:
        copy_file_to_dd(dirty_list_file, dump_dir, storage_unit_name=ddboost_storage_unit)
...
def write_partition_list_file(master_datadir, backup_dir, timestamp_key, master_port, dbname, dump_dir, dump_prefix,
    ...
    if ddboost:
        copy_file_to_dd(partition_list_file_name, dump_dir, storage_unit_name=ddboost_storage_unit)
...
class DumpDatabase(Operation):
    ...
    def create_dump_string(self, user_name, dump_date, timestamp_key):
        ...
        if self.ddboost_storage_unit:
            dump_line += " --ddboost_storage_unit_name=%s" % self.ddboost_storage_unit
        if self.incremental:
...
class DumpGlobal(Operation):
    def __init__(self, timestamp, master_datadir, master_port, backup_dir, dump_dir, dump_prefix, ddboost, ddboost_storage_unit):
        self.timestamp = timestamp
        self.master_datadir = master_datadir
        self.master_port = master_port
        self.backup_dir = backup_dir
    ...
    def execute(self):
        ...
        if self.ddboost:
            ...
            if self.ddboost_storage_unit:
                cmdStr += ' --storge_unit_name=%s' % self.ddboost_storage_unit
...
class DeleteCurrentDump(Operation):
    ...
    def execute(self):
        ...
        if self.ddboost:
            ...
            if self.ddboost_storage_unit:
                cmdStr += ' --storage_unit_name=%s' % self.ddboost_storage_unit
...
class DeleteOldestDumps(Operation):
    def __init__(self, master_datadir, master_port, dump_dir, cleanup_date=None, cleanup_total=None, ddboost=False, storage_unit_name=None):
        self.master_datadir = master_datadir
        self.master_port = master_port
        self.dump_dir = dump_dir
    ...
    def execute(self):
        ...
        if self.ddboost:
            cmdStr = 'gpddboost'
            if self.storage_unit_name:
                cmdStr += ' --storage_unit_name %s' % self.storage_unit_name 
...
""")
        ...
        if self.ddboost:
            ...
            logger.debug('Copying %s to DDBoost' % abspath)
            cmd = Command('DDBoost copy of %s' % abspath,
                          'gpddboost --storage_unit_name=%s --copyToDDBoost --from-file=%s --to-file=%s' % (self.ddboost_storage_unit, abspath, relpath))
            cmd.run(validateAfter=True)
